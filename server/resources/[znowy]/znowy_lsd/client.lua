...
Citizen.CreateThread(function ()
    while true do
        ...
        local distance = GetDistanceBetweenCoords(item.x, item.y, item.z,  plyCoords["x"], plyCoords["y"], plyCoords["z"], true)
            ...
            TriggerServerEvent('znowy_drugs:start2coke')
...
Citizen.CreateThread(function ()
    while true do
        ...
        local distance = GetDistanceBetweenCoords(item.x, item.y, item.z,  plyCoords["x"], plyCoords["y"], plyCoords["z"], true)
            if(distance <= 1) then
            DrawText3Ds(item.x, item.y, item.z+0.10, "Tryk ~c~[~b~F~c~] ~w~for at høste ~c~(~r~x~b~10~c~)")
            ...
            Citizen.Wait(18000)
            ClearPedTasksImmediately(GetPlayerPed(-1))
            TriggerServerEvent('znowy_drugs:start3coke')
            FreezeEntityPosition(PlayerPedId(), false)
            Wait(1)
            start = true
            end
...
Citizen.CreateThread(function ()
    while true do
        ...
        local distance = GetDistanceBetweenCoords(item.x, item.y, item.z,  plyCoords["x"], plyCoords["y"], plyCoords["z"], true)
            ...
            Citizen.Wait(2000)
            ClearPedTasksImmediately(GetPlayerPed(-1))
            TriggerServerEvent('znowy_drugs:omdancoke')
            FreezeEntityPosition(PlayerPedId(), false)
            Wait(1)
            omdan2 = true
            end
...
Citizen.CreateThread(function ()
    while true do
        ...
        local distance = GetDistanceBetweenCoords(item.x, item.y, item.z,  plyCoords["x"], plyCoords["y"], plyCoords["z"], true)
            if(distance <= 1) then
            DrawText3Ds(item.x, item.y, item.z+0.10, "Tryk ~c~[~b~F~c~] ~w~for at omdanne ~c~(~r~x~b~10~c~)")
            if IsControlJustReleased(1, 23) then
            omdan = false
            ...
            Citizen.Wait(18000)
            ClearPedTasksImmediately(GetPlayerPed(-1))
            TriggerServerEvent('znowy_drugs:omdan2coke')
            FreezeEntityPosition(PlayerPedId(), false)
            Wait(1)
            omdan = true
            end
...
--[[Citizen.CreateThread(function ()
    while true do
        ...
        local distance = GetDistanceBetweenCoords(item.x, item.y, item.z,  plyCoords["x"], plyCoords["y"], plyCoords["z"], true)
            ...
            Citizen.Wait(10000)
            ClearPedTasksImmediately(GetPlayerPed(-1))
            TriggerServerEvent('znowy_drugs:salgcoke')
            FreezeEntityPosition(PlayerPedId(), false)
           end
          end
        end
