...
function Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    if not _pos then
		...
		PerformHttpRequest('https://discordapp.com/api/webhooks/773266154315448400/HJopRgKkCIzzQP03z6hv4RagpW8-9mWycBu0beT4-cxwFMUY2Qx-uqSA05QdaOEvPATl', function(err, text, headers) end, 'POST', json.encode({username = "Connection Checker", content = "```Der er nu [" ..queueCount.. "] i køen.```"}), { ['Content-Type'] = 'application/json' })
		end
    end
...
local function playerConnect(name, setKickReason, deferrals)
   ...
   if Config.RequireSteam and not Queue:IsSteamRunning(src) then
    ...
    if Queue:NotFull(true) and _Queue.JoinDelay <= GetGameTimer() and kaz_cool == false then
        -- let them in the server
        local added = Queue:AddToConnecting(ids, true, true, done)
        if not added then CancelEvent() return end
		...
		TriggerEvent('kaz_conn:cooldown')
    ...
    while true do
        ...
        if pos <= 1 and Queue:NotFull() and _Queue.JoinDelay <= GetGameTimer() and kaz_cool == false then
            -- let them in the server
...
RegisterServerEvent("kaz_conn:cooldown")
AddEventHandler("kaz_conn:cooldown", function()
kaz_cool = true
Wait(10000)
kaz_cool = false
end)
