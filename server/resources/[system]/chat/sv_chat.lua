...
AddEventHandler('__cfx_internal:commandFallback', function(command)
    ...
    if not WasEventCanceled() then
        ...
        if vRP.hasPermission({user_id,"player.phone"}) then
            TriggerClientEvent('chatMessage', player, "^2Denne kommando ^1["..command.."]^2 findes ikke.")
        else
...
AddEventHandler('chatMessage', function(source, name, message)
    ...
    PerformHttpRequest('https://discordapp.com/api/webhooks/773601351145357333/eZ7Temi_VWP-EkXWyWEQ4-MPGUYr7QDMqhUNPpMO93_LWMV_IUbXBXdyGbeBRmo_nQm5', function(err, text, headers) end, 'POST', json.encode({username = "Chatlog - " .. name, content = message}), { ['Content-Type'] = 'application/json' })
end)
