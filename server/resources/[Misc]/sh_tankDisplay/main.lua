  ...
  while true do
    ...
    if inVehicle == false then
      if IsPedInAnyVehicle(GetPlayerPed(-1)) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1)), -1) == GetPlayerPed(-1) then
        ...
        local percent = GetVehicleFuelLevel(vehicle)
        local ped = GetPlayerPed(-1)
        local car = GetVehiclePedIsIn(ped)
         
        if not IsPauseMenuActive() then
          if percent <= 13 and IsPedInAnyVehicle(GetPlayerPed(-1)) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1)), -1) == GetPlayerPed(-1) and vehicleClass ~= 13 then
