RegisterNetEvent("wheelchair:spawn")
AddEventHandler("wheelchair:spawn", function()
	...
	while true do
		...
		if DoesEntityExist(closestObject) then
			...
			if GetDistanceBetweenCoords(pedCoords, sitCoords, true) <= 1.0 then
				DrawText3Ds(sitCoords, "~w~[~b~E~w~] Sid", 0.4)
			...
			if GetDistanceBetweenCoords(pedCoords, pickupCoords, true) <= 1.0 then
				DrawText3Ds(pickupCoords, "~w~[~b~E~w~] Tag op", 0.4)
