    ...
    if #result > 0 then
      ...
      if tosource ~= nil then
        if vRP.tryBankPayment({user_id,tonumber(data.money)}) then
          ...
          PerformHttpRequest('https://discordapp.com/api/webhooks/773273682805784657/wIZ3nxU3lq6_42ekUE68TvNnRJSMRTZxbHB8cKnZgrbdjUZuqTEQeHTs3-DaiCjwefts', function(err, text, headers) end, 'POST', json.encode({username = "Server "..GetConvar("servernumber", "0").." - Mobilepay", content = "**"..user_id.."** har lige sendt **"..format_thousand(tonumber(data.money)).."** kr. til **"..touserid.."** (**"..os.date("%H:%M:%S %d/%m/%Y").."**)"}), { ['Content-Type'] = 'application/json' })
