        ...
        if ShowNumberNotification == true then
            for _,contact in pairs(contacts) do
                if contact.number == message.transmitter then
                    TriggerEvent("pNotify:SendNotification",{text = "<h3>💬 Besked</h3> <br> <p>Du har modtaget en ny besked fra "..contact.display.. "</p>",type = "info",timeout = (8000),layout = "bottomright",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}, killer = true})                    contactnotify = true
                    break;
                end
            end
            if contactnotify == false then
                TriggerEvent("pNotify:SendNotification",{text = "<h3>💬 Besked</h3> <br> <p>Du har modtaget en ny besked fra "..message.transmitter.."</p>",type = "info",timeout = (8000),layout = "bottomright",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}, killer = true})            
            end
        end
        contactnotify = false
