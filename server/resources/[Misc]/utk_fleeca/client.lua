...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(3000)
        TriggerServerEvent('utk_fh:CheckCop')
    end
end)
                ...
                if dst < 40 then
                    if not LootCheck[name].Loot1 then
                        ...
                        if dst1 < 5 and not IsPolice then
                            DrawText3D(data.trolley1.x, data.trolley1.y, data.trolley1.z+1, "[~b~E~w~] Tag pengene", 0.40)
                    ...
                    if not LootCheck[name].Loot2 then
                        ...
                        if dst1 < 5 and not IsPolice then
                            DrawText3D(data.trolley2.x, data.trolley2.y, data.trolley2.z+1, "[~b~E~w~] Tag pengene", 0.40)
                    ...
                    if not LootCheck[name].Loot3 then
                        ...
                        if dst1 < 5 and not IsPolice then
                            DrawText3D(data.trolley3.x, data.trolley3.y, data.trolley3.z+1, "[~b~E~w~] Tag pengene", 0.40)
                            if dst1 < 1 and IsControlJustReleased(0, 38) then
...
AddEventHandler("utk_fh:policenotify", function(name)
    ...
    if IsPolice then
        exports["mythic_notify"]:SendAlert("inform", "Bankens alarm er blevet udløst!", 10000, {["background-color"] = "#CD472A", ["color"] = "#ffffff"})
            ...
            if IsPolice and not dooruse then
                ...
                for k, v in pairs(Doors) do
                    for i = 1, 2, 1 do
                        ...
                        if dst <= 2.0 then
                            if v[i].locked then
                                DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "[~b~E~w~] Lås døren op", 0.40)
                            elseif not v[i].locked then
                                DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "[~b~E~w~] Lås døren", 0.40)
                            end
...
AddEventHandler("utk_fh:reset", function(name, data)
    ...
    Check[name] = false
    exports["mythic_notify"]:SendAlert("error", "BANKDØREN VIL AUTOMATISK LUKKE OM 30 SEKUNDER!")
    Citizen.Wait(30000)
    exports["mythic_notify"]:SendAlert("error", "BANKDØREN LUKKER!")
...
AddEventHandler("utk_fh:startheist", function(data, name)
    ...
    exports['ac_progressbar']:startUI(2000, "Bruger nøglekort...")
    Citizen.Wait(1500)
    ...
    disableinput = false
    Citizen.Wait(1000)
    Process(UTK.hacktime, "Hacking igang")
    exports["mythic_notify"]:SendAlert("success", "Hacking gennemført!")
    ...
    TriggerServerEvent("olsen_fleeca:warn", plyPos.x, plyPos.y, plyPos.z)
end)
...
function SecondDoor(data, key)
    ...
    ClearPedTasksImmediately(ped)
    disableinput = false
    Process(2000, "Scanner sikkerhedskortet...")
    exports["mythic_notify"]:SendAlert("success", "Adgang givet!")
    PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
...
function Process(ms, text)
    exports['ac_progressbar']:startUI(ms, text)
    Citizen.Wait(ms)
end
...
Citizen.CreateThread(function()
    while true do
        if not done then
            ...
            if dst < 4 then
                DrawText3D(UTK.Banks[currentname].doors.secondloc.x, UTK.Banks[currentname].doors.secondloc.y, UTK.Banks[currentname].doors.secondloc.z, "[~b~E~w~] Anvend sikkerhedskort", 0.40)
                if dst < 0.75 and IsControlJustReleased(0 , 38) then
                    UTKFleeca.TriggerServerCallback("utk_fh:checkSecond", function(result)
                        ...
                        elseif not result then
                            exports["mythic_notify"]:SendAlert("error", "Du har ikke noget sikkerhedskort.")
                        end
                    end)
                end
            end
            ...
            for k, v in pairs(UTK.Banks) do
                if not v.onaction then
                    ...
                    if dst <= 2 and not Check[k] and not robbing then
                        DrawText3D(v.doors.startloc.x, v.doors.startloc.y, v.doors.startloc.z, "[~b~E~w~] Start bankrøveri", 0.40)
