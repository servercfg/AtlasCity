...
AddEventHandler('es_bank:toofar', function(robb)
	if(robbers[source])then
		...
		PerformHttpRequest('https://discordapp.com/api/webhooks/773266657534935090/wooYsiBBQpldGhOSXhiKtbHZ7B8IO1u5lTS1UQHL064URi3ekn_0NnvpZuAtkc7Hae07', function(err, text, headers) end, 'POST', json.encode({username = "Røveri Logs", content = "**ID: "..user_id.." ** gik for langt væk fra bank'en "..bank.nameofbank.." som betjent!"}), { ['Content-Type'] = 'application/json' })
...
AddEventHandler('es_bank:playerdied', function(robb)
	if(robbers[source])then
		...
		local source = source
		local user_id = vRP.getUserId({source})
		PerformHttpRequest('https://discordapp.com/api/webhooks/773266657534935090/wooYsiBBQpldGhOSXhiKtbHZ7B8IO1u5lTS1UQHL064URi3ekn_0NnvpZuAtkc7Hae07', function(err, text, headers) end, 'POST', json.encode({username = "Røveri Logs", content = "**ID: "..user_id.." ** døde til et bankrøveri ved "..bank.nameofbank.."!"}), { ['Content-Type'] = 'application/json' })
		TriggerClientEvent("pNotify:SendNotification", -1,{text = "Bankrøveriet blev aflyst: " .. banks[robb].nameofbank, type = "warning", queue = "global", timeout = 5000, layout = "centerRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
	end
end)
...
AddEventHandler('es_bank:rob', function(robb)
	...
	if vRP.hasPermission({user_id,cfg.permission}) then
		...
		local source = source
		local user_id = vRP.getUserId({source})
		PerformHttpRequest('https://discordapp.com/api/webhooks/773266657534935090/wooYsiBBQpldGhOSXhiKtbHZ7B8IO1u5lTS1UQHL064URi3ekn_0NnvpZuAtkc7Hae07', function(err, text, headers) end, 'POST', json.encode({username = "Røveri Logs", content = "**ID: "..user_id.." ** prøvede at lave et bankrøveri ved "..bank.nameofbank.." som betjent!"}), { ['Content-Type'] = 'application/json' })
	else
		...
		if #cops >= ncops then
			if banks[robb] then
				...
				local user_id = vRP.getUserId({source})
				local player = vRP.getUserSource({user_id})
				PerformHttpRequest('https://discordapp.com/api/webhooks/773266657534935090/wooYsiBBQpldGhOSXhiKtbHZ7B8IO1u5lTS1UQHL064URi3ekn_0NnvpZuAtkc7Hae07', function(err, text, headers) end, 'POST', json.encode({username = "Røveri Logs", content = "**ID: "..user_id.." ** Startede lige et bankrøveri ved "..bank.nameofbank.."!"}), { ['Content-Type'] = 'application/json' })
				...
				SetTimeout(cfg.seconds*1000, function()
					if(robbers[savedSource])then
						if(user_id)then
							...
							local source = source
							local user_id = vRP.getUserId({source})
							PerformHttpRequest('https://discordapp.com/api/webhooks/773266657534935090/wooYsiBBQpldGhOSXhiKtbHZ7B8IO1u5lTS1UQHL064URi3ekn_0NnvpZuAtkc7Hae07', function(err, text, headers) end, 'POST', json.encode({username = "Røveri Logs", content = "**ID: "..user_id.." **'s bankrøveri blev gennemført ved "..bank.nameofbank.."!"}), { ['Content-Type'] = 'application/json' })
