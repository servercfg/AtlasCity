...
MySQL.createCommand("vRP/update_user_identity","UPDATE vrp_user_identities SET firstname = @firstname, name = @name, age = @age, phone = @phone WHERE user_id = @user_id")
...
MySQL.createCommand("vRP/status_driverlicense", "SELECT DmvTest FROM vrp_users WHERE id = @user_id")
...
function vRP.generateRegistrationNumber(cbr)
  ...
  local function search()
    ...
    local registration1 = vRP.generateStringNumber("DDDDD") --CPR
	local registration2 = "1"
	local registration3 = registration2..registration1
    vRP.getUserByRegistration(registration3, function(user_id)
      if user_id ~= nil then
...
AddEventHandler("vRP:playerJoin",function(user_id,source,name,last_login)
  vRP.getUserIdentity(user_id, function(identity)
    if identity == nil then
      --vRP.giveInventoryItem(user_id, "firstlogin", 1, true)
        ...
        vRP.prompt(player,lang.cityhall.identity.prompt_name(),"",function(player,name)
          if string.len(name) >= 2 and string.len(name) < 50 then
            ...
            vRP.prompt(player,lang.cityhall.identity.prompt_age(),"",function(player,age)
              ...
              if age >= 16 and age <= 150 then
                if vRP.tryPayment(user_id,cfg.new_identity_cost) then
					if vRP.tryGetInventoryItem(user_id,"firstlogin",1,false) then
					  --vRP.generateRegistrationNumber(function(registration)
						vRP.generatePhoneNumber(function(phone)
            ...
            --end)
						 ...
						 TriggerClientEvent("pNotify:SendNotification", player,{text = "Du mangler en dåbsattest", type = "error", queue = "global", timeout = 4000, layout = "centerRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
					end
                else
