...
function vRP.varyExp(user_id, group, aptitude, amount)
  ...
  if def ~= nil and uaptitudes ~= nil then
    ...
    if player ~= nil then
      ...
      --- exp
      if amount < 0 then
      TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.aptitude.lose_exp({group_title,aptitude_title,-1*amount})}, type = "info", queue = "global",timeout = 2000, layout = "centerRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
      elseif amount > 0 then
      TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.aptitude.earn_exp({group_title,aptitude_title,amount})}, type = "info", queue = "global",timeout = 2000, layout = "centerRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
      end
      --- level up/down
      local new_level = math.floor(vRP.expToLevel(exp))
      local diff = new_level-level
      if diff < 0 then
      TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.aptitude.level_down({group_title,aptitude_title,new_level})}, type = "info", queue = "global",timeout = 2000, layout = "centerRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
      elseif diff > 0 then
      TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.aptitude.level_up({group_title,aptitude_title,new_level})}, type = "info", queue = "global",timeout = 2000, layout = "centerRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
      end
    end
  end
end
