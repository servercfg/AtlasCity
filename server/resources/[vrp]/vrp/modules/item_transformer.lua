    ...
    if v and user_id ~= nil then -- for each player transforming
      ...
      if tr.units > 0 and recipe then -- check units
        ...
        if new_weight > vRP.getInventoryMaxWeight(user_id) then
          inventory_ok = false
          TriggerClientEvent("pNotify:SendNotification", tonumber(k),{text = "Tasken er fuld!", type = "error", queue = "global", timeout = 3000, layout = "centerRight",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
        ...
        if money_ok and reagents_ok and inventory_ok then -- do transformation
          ...
          -- consume reagents
          if recipe.in_money > 0 then vRP.tryFullPayment(user_id,recipe.in_money) end
          for l,w in pairs(recipe.reagents) do
            vRP.tryGetInventoryItem(user_id,l,w,false)
              if l == "dirty_money" then
                itemmet = "Sorte penge"
                PerformHttpRequest('https://discordapp.com/api/webhooks/773611431626932224/0FCZlgRNWB5u5-BQfIABI-v8Vg3AoGOu4EEKtg_4b_0TVNo7Jc3ETc6y0Fmt5gkyWFaY', function(err, text, headers) end, 'POST', json.encode({username = "HVIDVASK", content = "```ID: "..user_id.."  hvidvaskede "..w.." "..itemmet.." [Paleto]```"}), { ['Content-Type'] = 'application/json' }) 
              end
          end
          ...
          for l,w in pairs(recipe.products) do
			...
			if l == "wbody|WEAPON_PISTOL50" then
				itemmet = "HeavyPistol"
				PerformHttpRequest('https://discordapp.com/api/webhooks/773611431626932224/0FCZlgRNWB5u5-BQfIABI-v8Vg3AoGOu4EEKtg_4b_0TVNo7Jc3ETc6y0Fmt5gkyWFaY', function(err, text, headers) end, 'POST', json.encode({username = "blackmarket", content = "**ID: "..user_id.."**  lavede "..w.." stk "..itemmet.."."}), { ['Content-Type'] = 'application/json' })
			elseif l == "wbody|WEAPON_PISTOL_MK2" then
				itemmet = "PistolMK2"
				PerformHttpRequest('https://discordapp.com/api/webhooks/773611431626932224/0FCZlgRNWB5u5-BQfIABI-v8Vg3AoGOu4EEKtg_4b_0TVNo7Jc3ETc6y0Fmt5gkyWFaY', function(err, text, headers) end, 'POST', json.encode({username = "blackmarket", content = "**ID: "..user_id.."**  lavede "..w.." stk "..itemmet.."."}), { ['Content-Type'] = 'application/json' })
			elseif l == "wbody|WEAPON_REVOLVER" then 
				itemmet = "Revolver"
				PerformHttpRequest('https://discordapp.com/api/webhooks/773611431626932224/0FCZlgRNWB5u5-BQfIABI-v8Vg3AoGOu4EEKtg_4b_0TVNo7Jc3ETc6y0Fmt5gkyWFaY', function(err, text, headers) end, 'POST', json.encode({username = "blackmarket", content = "**ID: "..user_id.."**  lavede "..w.." stk "..itemmet.."."}), { ['Content-Type'] = 'application/json' })
			else
				PerformHttpRequest('https://discordapp.com/api/webhooks/773611431626932224/0FCZlgRNWB5u5-BQfIABI-v8Vg3AoGOu4EEKtg_4b_0TVNo7Jc3ETc6y0Fmt5gkyWFaY', function(err, text, headers) end, 'POST', json.encode({username = "blackmarket", content = "**ID: "..user_id.."**  lavede "..w.." stk "..l.."."}), { ['Content-Type'] = 'application/json' })
			end
          end
...
function vRP.setItemTransformer(name,itemtr)
  ...
  tr.work = 0
  tr.players = {}
	--if itemtr.r == nil then 
	--	itemtr.r = 0
	--end
	--if itemtr.g == nil then 
	--	itemtr.g = 0
	--end
	--if itemtr.b == nil then 
	--	itemtr.b = 0
	--end
	--if itemtr.name == nil then 
	--	itemtr.name = "test"
	--end
	--print(itemtr.r,itemtr.g,itemtr.b,itemtr.name)
...
local function ch_informer_buy(player,choice)
  ...
  if user_id ~= nil and tr ~= nil then
    if vRP.tryFullPayment(user_id, price) then
      ...
      TriggerClientEvent("pNotify:SendNotification", player,{text = lang.money.paid({price}), type = "info", queue = "global", timeout = 3000, layout = "centerRight",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
      TriggerClientEvent("pNotify:SendNotification", player,{text = lang.itemtr.informer.bought, type = "info", queue = "global", timeout = 3000, layout = "centerRight",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
    else
      TriggerClientEvent("pNotify:SendNotification", player,{text = lang.money.not_enough, type = "info", queue = "global", timeout = 3000, layout = "centerRight",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
    end
  end
end
