...
AddEventHandler("vRP:playerSpawn", function(user_id, source, first_spawn)
  ...
  if first_spawn then -- first spawn
    ...
    -- notify last login
    SetTimeout(15000,function()TriggerClientEvent("pNotify:SendNotification", player,{text = {lang.common.welcome({tmpdata.last_login})}, type = "backside", queue = "global", timeout = 10000, layout = "Center",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})end)
    ...
    if cfg.spawn_enabled then -- respawn
      ...
      local z = cfg.spawn_position[3]+math.random()*cfg.spawn_radius*2-cfg.spawn_radius
      data.position = {x=x,y=y,z=z}
      vRPclient.teleport(source,{300.04934692383,-577.64465332031,43.260837554932})
		TriggerClientEvent('movebitch',player)
    end
