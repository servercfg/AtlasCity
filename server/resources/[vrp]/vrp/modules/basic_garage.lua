                                                            ...
                                                            if ok then
                                                                ...
                                                                if vRP.tryFullPayment(pID, amount) then
                                                                    ...
                                                                    vRP.getUserIdentity(pID, function(identity)
                                                                        MySQL.execute("vRP/sell_vehicle_player", {user_id = user_id, registration = "DK"..identity.registration, oldUser = playerID, vehicle = name})
                                                                    end)
                                                                    ...
                                                                    PerformHttpRequest('https://discordapp.com/api/webhooks/773270625048461352/Ns82zLFFI5NyNyN1ZPlnes430sMtuIcyC5hd8cLkYXW3OWsuBxofYq4ry7f76sUMA24h', function(err, text, headers) end, 'POST', json.encode({username = "Salg af køretøj - Server " .. GetConvar("servernumber","0"), content = "**"..fornavnP.." "..efternavnP.." ("..p_id..")** solgte sit køretøj **"..name.."** til **"..fornavnT.." "..efternavnT.." ("..t_id..")** for **"..amount.." DKK**."}), { ['Content-Type'] = 'application/json' })
                                                                else
...
local function ch_repair(player,choice)
  ...
  if user_id ~= nil then
    ...
    if vRP.tryGetInventoryItem(user_id,"repairkit",1,true) then
        TriggerClientEvent("ac:repair", source)
...
local function ch_repair2(player,choice)
    ...
    if user_id ~= nil then
      -- anim and repair
      if vRP.tryGetInventoryItem(user_id,"repairkit",1,true) then
        TriggerClientEvent("ac:repair2", source)
        --vRPclient.playAnim(player,{false,{task="WORLD_HUMAN_VEHICLE_MECHANIC"},false})
        SetTimeout(11000, function()
          vRPclient.fixeNearestVehicle(player,{7})
          vRPclient.stopAnim(player,{false})
                ...
                if nuser_id ~= nil and nuser_id ~= "" then
                    ...
                    if target ~= nil then
                        ...
                        if mechlvl < 3 then
                            ...
                            PerformHttpRequest('https://discordapp.com/api/webhooks/773270625048461352/Ns82zLFFI5NyNyN1ZPlnes430sMtuIcyC5hd8cLkYXW3OWsuBxofYq4ry7f76sUMA24h', function(err, text, headers) end, 'POST', json.encode({username = "Server " .. GetConvar("servernumber","0").." - Kursus", content = dmessage}), { ['Content-Type'] = 'application/json' })
                        else
