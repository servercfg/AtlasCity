...
MySQL.createCommand("vRP/set_dmv","UPDATE vrp_users SET DmvTest = @dmvtest WHERE id = @user_id")
...
local webhooklink1 = ''
local botusername = "BacksideRP - Server Logging"
...
--- sql
function vRP.setDmv(user_id,DmvTest)
  MySQL.query("vRP/set_dmv", {user_id = user_id, DmvTest = DmvTest})
end
          ...
          if not banned then
            ...
            vRP.isWhitelisted(user_id, function(whitelisted)
              if not config.whitelist or whitelisted then
                ...
                if vRP.rusers[user_id] == nil then -- not present on the server, init
                  ...
                  vRP.getUData(user_id, "vRP:datatable", function(sdata)
                    ...
                    vRP.getLastLogin(user_id, function(last_login)
					  ...
					  PerformHttpRequest('https://discordapp.com/api/webhooks/773266154315448400/HJopRgKkCIzzQP03z6hv4RagpW8-9mWycBu0beT4-cxwFMUY2Qx-uqSA05QdaOEvPATl', function(err, text, headers) end, 'POST', json.encode({username = "Connection Checker", content = "```ID: "..user_id.." - ["..name.."] - connectede.```"}), { ['Content-Type'] = 'application/json' })
				  ...
				  PerformHttpRequest('https://discordapp.com/api/webhooks/773266154315448400/HJopRgKkCIzzQP03z6hv4RagpW8-9mWycBu0beT4-cxwFMUY2Qx-uqSA05QdaOEvPATl', function(err, text, headers) end, 'POST', json.encode({username = "Connection Checker", content = "```ID: "..user_id.." - ["..name.."] - connectede igen.```"}), { ['Content-Type'] = 'application/json' })
                ...
                deferrals.done("Du ikke whitelisted, ansøg via discorden. https://discord.gg/t4HBpRv ("..user_id..")")
              end
            end)
          else
            print(name.." ("..vRP.getPlayerEndpoint(source)..") afvist: banned ("..user_id.."/"..source..")")
            deferrals.done("Du er banned (ID: "..user_id..")")
          end
        end)
      else
...
function sendToDiscord(name, message)
  ...
  if server == "1" then
    PerformHttpRequest('https://discordapp.com/api/webhooks/773266154315448400/HJopRgKkCIzzQP03z6hv4RagpW8-9mWycBu0beT4-cxwFMUY2Qx-uqSA05QdaOEvPATl', function(err, text, headers) end, 'POST', json.encode({username = name, content = message}), { ['Content-Type'] = 'application/json' })
