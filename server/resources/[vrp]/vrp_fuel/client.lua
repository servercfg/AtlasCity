...
isNearFuelStation, stationNumber = 0
isNearFuelPStation, stationPlaneNumber = 0
isNearFuelHStation, stationHeliNumber = 0
isNearFuelBStation, stationBoatNumber = 0
...
Citizen.CreateThread(function()
	...
	if isNearFuelStation ~= 0 or isNearFuelPStation ~= 0 or isNearFuelHStation ~= 0 or isNearFuelBStation ~= 0 then 
	...
	local isEngineOn = Citizen.InvokeNative(0xAE31E7DF9B5B132E, GetVehiclePedIsIn(GetPlayerPed(-1)))
		...
		if isEngineOn then
			if isRadarExtended then
				...
				DrawAdvancedText(0.5050, 0.9550, -0.277, 0.0028, 0.42,round(percent,1), 255, 255, 255, 220, 0, 1)
			...
			DrawAdvancedText(0.5050, 0.9550, -0.260, 0.0028, 0.42,"%", 255, 255, 255, 220, 0, 1)
		end
    end
end
...
AddEventHandler("essence:hasBuying", function(amount, toPay)
	TriggerEvent("pNotify:SendNotification",{text = "Du har købt "..amount.." liter brændstof for <b style='color: #DB4646'>"..toPay.." DKK</b>",type = "success",timeout = (4000),layout = "centerRight",queue = "global",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
