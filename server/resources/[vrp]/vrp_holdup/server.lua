...
AddEventHandler("playerDropped", function()
	if(robbers[source])then
        ...
        PerformHttpRequest('https://discordapp.com/api/webhooks/773266657534935090/wooYsiBBQpldGhOSXhiKtbHZ7B8IO1u5lTS1UQHL064URi3ekn_0NnvpZuAtkc7Hae07', function(err, text, headers) end, 'POST', json.encode({username = "Røveri Logs", content = "**ID: "..user_id.." ** afbrød et røveri ved "..homoseksuelt.."!"}), { ['Content-Type'] = 'application/json' })
...
AddEventHandler('vrp_holdup:toofar', function(robb)
	if(robbers[source])then
		...
		local source = source
		local user_id = vRP.getUserId({source})
        PerformHttpRequest('https://discordapp.com/api/webhooks/773266657534935090/wooYsiBBQpldGhOSXhiKtbHZ7B8IO1u5lTS1UQHL064URi3ekn_0NnvpZuAtkc7Hae07', function(err, text, headers) end, 'POST', json.encode({username = "Røveri Logs", content = "**ID: "..user_id.." ** afbrød et røveri ved "..stores[robb].nameofstore.."!"}), { ['Content-Type'] = 'application/json' })
...
AddEventHandler('vrp_holdup:rob', function(robb,x,y,z)
  ...
  if vRP.hasPermission({user_id,cfg.permission}) then
	local source = source
	local user_id = vRP.getUserId({source})
	PerformHttpRequest('https://discordapp.com/api/webhooks/773266657534935090/wooYsiBBQpldGhOSXhiKtbHZ7B8IO1u5lTS1UQHL064URi3ekn_0NnvpZuAtkc7Hae07', function(err, text, headers) end, 'POST', json.encode({username = "Røveri Logs", content = "**ID: "..user_id.." ** prøve at røve "..stores[robb].nameofstore.." som betjent!"}), { ['Content-Type'] = 'application/json' })
    ...
    if #cops >= cfg.cops then
	  if stores[robb] then
		  ...
		  local source = source
		  local user_id = vRP.getUserId({source})
		  PerformHttpRequest('https://discordapp.com/api/webhooks/773266657534935090/wooYsiBBQpldGhOSXhiKtbHZ7B8IO1u5lTS1UQHL064URi3ekn_0NnvpZuAtkc7Hae07', function(err, text, headers) end, 'POST', json.encode({username = "Røveri Logs", content = "**ID: "..user_id.." ** startede et røveri ved "..stores[robb].nameofstore.."!"}), { ['Content-Type'] = 'application/json' }) 
		  ...
		  SetTimeout(cfg.seconds*1000, function()
			  if(robbers[savedSource])then
				  if(user_id)then
					  ...
					  local source = source
					  local user_id = vRP.getUserId({source})
					  PerformHttpRequest('https://discordapp.com/api/webhooks/773266657534935090/wooYsiBBQpldGhOSXhiKtbHZ7B8IO1u5lTS1UQHL064URi3ekn_0NnvpZuAtkc7Hae07', function(err, text, headers) end, 'POST', json.encode({username = "Røveri Logs", content = "**ID: "..user_id.." **'s Røveri ved "..store.nameofstore.." blev afsluttet, røverne slap væk med ca. "..store.reward.." i kontanter"}), { ['Content-Type'] = 'application/json' }) 
