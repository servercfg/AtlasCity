...
function updateTokovoipInfo(msg) {
	...
	switch (voipStatus) {
		...
		case PLUGIN_INITIALIZING:
			...
			break;
		case WRONG_SERVER:
			msg = `Tilsluttet den forkerte TeamSpeak server, join venligst den rigtige server: <font color="#01b0f0">${voip.plugin_data.TSServer}</font>`;
			screenMessage = 'Forkert TeamSpeak server';
			color = 'red';
			break;
		case WRONG_CHANNEL:
			msg = `Tilsluttet den forkerte TeamSpeak kanal, join venligst den rigtige kanal: <font color="#01b0f0">${voip.plugin_data.TSChannelWait && voip.plugin_data.TSChannelWait !== '' && voip.plugin_data.TSChannelWait || voip.plugin_data.TSChannel}</font>`;
			screenMessage = 'Forkert TeamSpeak kanal';
			color = 'red';
			break;
		case INCORRECT_VERSION:
			msg = 'Du gør brug af forkert plugin version';
			screenMessage = 'Forkert plugin version';
			color = 'red';
			break;
		case OK:
			color = '#01b0f0';
	...
	if (msg) {
		document.getElementById('tokovoipInfo').innerHTML = `<font color="${color}">[BS-Voice] ${msg}</font> 🔊`;
	}
...
function updateConfig(payload) {
	...
	document.getElementById('TSChannel').innerHTML = `TeamSpeak kanal: <font color="#01b0f0">${(voip.plugin_data.TSChannelWait) ? voip.plugin_data.TSChannelWait : voip.plugin_data.TSChannel}</font>`;
